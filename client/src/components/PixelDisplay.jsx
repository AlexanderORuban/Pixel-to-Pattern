...
export default function PixelDisplay({ patternInfo, displayWidth, displayHeight }) {
    ...
    useEffect(() => {
        // Validate pattern dimensions before doing any canvas work
        if (!patternWidth || !patternHeight || isNaN(patternWidth) || isNaN(patternHeight)) {
            console.warn("Invalid pattern dimensions:", patternWidth, patternHeight);
            return; // stop if we don’t have valid numbers
        }
    ...
    const changeCanvas = (canvasData) => {
        if (!Array.isArray(colorConfig)) {
            console.warn("colorConfig is not an array:", colorConfig);
            return;
        }

        let j = 0;
        for (let i = 0; i < canvasData.length && j < colorConfig.length; i += 4) {
